...
// Premiere paire de coords trouvee (pour detecter l'ordre des axes)
function firstPair(coords) {
  if (!coords) return null;
  if (typeof coords[0] === 'number') return coords;
  return firstPair(coords[0]);
}
// France metropolitaine : lon ∈ [-6,11], lat ∈ [41,52]. Si on trouve [lat,lon] -> on inverse.
function looksSwapped(geom) {
  var fp = geom && geom.coordinates ? firstPair(geom.coordinates) : null;
  if (!fp) return false;
  var a = fp[0], b = fp[1];
  var normal  = (a >= -6 && a <= 11) && (b >= 41 && b <= 52);
  var swapped = (a >= 41 && a <= 52) && (b >= -6 && b <= 11);
  return swapped && !normal;
}
function swapAxes(geom) {
  if (!geom || !geom.coordinates) return geom;
  function sw(c) { return typeof c[0] === 'number' ? [c[1], c[0]] : c.map(sw); }
  return { type: geom.type, coordinates: geom.coordinates.map(sw) };
}
...
function extractRecords(raw) {
  ...
  if (raw.values) return raw.values;  // data.grandlyon datapusher all.json
...
function build(ville, inputPath, outPath, overridePath) {
  ...
  var quartiers = {}, loyers = {}, kept = 0, AXIS_WARNED = false;
  recs.forEach(function (rec) {
    ...
    if (!quartiers[qid]) {
      var g = geomFor(rec);
      if (g) {
        if (looksSwapped(g)) { g = swapAxes(g); if (!AXIS_WARNED) { console.log('  ↻ coordonnees lat,lon detectees → inversees en lon,lat'); AXIS_WARNED = true; } }
        quartiers[qid] = { id: qid, nom: val(rec, 'nom_quartier') || qid, zone: val(rec, 'id_zone'), geometry: round6(g) };
      }
    }
